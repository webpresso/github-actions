...
- unresolved-count: 0
- verified-at: 2026-08-23T21:04:10.000Z
...
| D3 | Regression style | Permanent semantic source-to-prose parity through M1-M6 plus action non-description and freshness byte contracts; repository-wide changed-path scope is proven once as canonical integration evidence. | A permanent task-start repo diff; snapshot whole prose files; presence-only substring checks; hand-maintained unsupported inventory; prose-only change without RED proof. | Durable product contracts belong in Bun tests. A task-scoped repo-wide allowlist is meaningful only for this delivery and would become stale after later legitimate repository changes, so it belongs in one-time task evidence. |
...
| D6 | No-behavior-change proof | Keep permanent focused regressions for the action's non-description projection and the freshness workflow bytes, and run the task-start changed-path allowlist exactly once as canonical integration evidence before verification. | Embed a permanent repo-wide diff against a frozen SHA; rely only on suite greens; manually eyeball the final diff; permit action name, inputs, runs.steps, or workflow drift. | The action/workflow contracts remain useful after this task, while the repository-wide path delta is delivery-specific. Separating them preserves durable protection without making future legitimate changes fail an obsolete scope test. |

### Promotion Gates

| Gate | Command | Expected outcome | Last result | Defer |
| --- | --- | --- | --- | --- |
| GitHub Actions secret-reference audit | wp audit github-actions-secrets | GitHub Actions workflows and action metadata satisfy repository secret-reference policy. | deferred: pre-implementation (explicit defer marker) | pre-implementation |
